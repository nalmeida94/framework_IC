...
class SerialPort():
    def __init__(self, nodeSync, path):
        ...
        self.eventSerial = MyEvent(self.event, self.ser)
        self.listenerSerial = MyListener(self.event, self.ser, self.nodeSync)
    ...
    def sendMessage(self):
        while(1):
            ...
            if(msg=='exit'):
                ...
                self.eventSerial.running.set()
                self.listenerSerial.running.set()
                self.ser.close()
                sys.exit()    
            try:        
                #Writing the message
